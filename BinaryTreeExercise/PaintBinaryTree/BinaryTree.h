...
private:
    ...
    int calculateHeight(TreeNode<T>* node) const {
        ...
        int leftHeight = calculateHeight(node->leftChild);  // 左子树高度
        int rightHeight = calculateHeight(node->rightChild); // 右子树高度
        return 1 + std::max(leftHeight, rightHeight);       // 当前高度为子树高度的最大值加1
    }
public:
